...
public class InwardDaoImpl implements InwardDao{
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Inward> getInwardByOrgId(String organisationId) {
		System.out.println("inside getMaterialByOrgId() method");
		Session hbSession = sFactory.getCurrentSession();
		Query query = hbSession.createSQLQuery("select * from inwardmaster where organisation_id=:organisationId")
				.addEntity(Inward.class);
