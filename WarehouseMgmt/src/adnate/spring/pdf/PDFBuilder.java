...
/*private static String FILE = "D:/received_projects/final_proj_till_date/WarehouseMgmt123/Resources/";
*/	
    ...
    public static void pdfCreation(List<InvoiceDetails> invoiceDetails, List<InvoiceTax> taxDetailsList, File newPdfFile) throws MalformedURLException, IOException {
	...
	try {
           PdfWriter pdfWriter=PdfWriter.getInstance(document, new FileOutputStream(newPdfFile));
    ...
    public static class HeaderFooter extends PdfPageEventHelper{
    	 public void onStartPage(PdfWriter pdfWriter, Document document) {
    	      ...
    	      try {
				Image image=Image.getInstance("D:/received_projects/final_proj_till_date/WarehouseMgmt123/Resources/adnate.png");
				document.add(image);
	 ...
	 private static void addContent(Document document,List<InvoiceDetails> invoiceDetails, List<InvoiceTax> taxDetailsList) throws DocumentException, MalformedURLException, IOException{
		    ...
		    String invoiceId="";
		     ...
		     for(InvoiceDetails invoice:invoiceDetails){
		    	 invoiceId=String.valueOf(invoice.getInvoice().getInvoiceId());
			    	...
			    	subTotal1=String.valueOf(precision.format(invoice.getInvoice().getInvoiceAmtTotal()));
			    	discount1=String.valueOf(precision.format(invoice.getInvoice().getDiscount()));
			    }
		    ...
		    double subTot=0.0;
		    	for(InvoiceDetails invoice:invoiceDetails){
		    		 PdfPCell itemdata=new PdfPCell(new Phrase(invoice.getMaterial().getMaterialName(),batFont));
			             ...
			             PdfPCell totalCostdata=new PdfPCell(new Phrase(String.valueOf(precision.format(invoice.getTotalAmount())),batFont));
		    ...
		    for(InvoiceTax invoiceTax:taxDetailsList){
		    	if(invoiceTax.getTax().getTaxName().equals("CST")){
				    ...
				    PdfPCell cst=new PdfPCell(new Phrase("CST("+String.valueOf(precision.format(invoiceTax.getTax().getTaxValue()))+")",batFont));
				    cst.setHorizontalAlignment(Element.ALIGN_RIGHT);
				    cst.setBorder(Rectangle.NO_BORDER);
				    subChildTable.addCell(cst);
				   
				    
				    PdfPCell cstData=new PdfPCell(new Phrase(String.valueOf(precision.format(invoiceTax.getTaxableAmt())),batFont));
				    cstData.setHorizontalAlignment(Element.ALIGN_RIGHT);
				    cstData.setBorder(Rectangle.NO_BORDER);
			        subChildTable.addCell(cstData);
		    	}
		    	
		    	if(invoiceTax.getTax().getTaxName().equals("Swachh Bharat")){
		    		System.out.println("faskb" + invoiceTax.getTax().getTaxName());
		    		subChildTable.addCell(blank);
				    subChildTable.addCell(blank);
				    
				    PdfPCell sb=new PdfPCell(new Phrase("Swachh Bharat("+String.valueOf(precision.format(invoiceTax.getTax().getTaxValue()))+")",batFont));
				    sb.setHorizontalAlignment(Element.ALIGN_RIGHT);
				    sb.setBorder(Rectangle.NO_BORDER);
				    subChildTable.addCell(sb);
				   
				    
				    PdfPCell sbData=new PdfPCell(new Phrase(String.valueOf(precision.format(invoiceTax.getTaxableAmt())),batFont));
		    	...
		    	if(invoiceTax.getTax().getTaxName().equals("VAT")){
				    ...
				    PdfPCell vat=new PdfPCell(new Phrase("VAT("+String.valueOf(precision.format(invoiceTax.getTax().getTaxValue()))+")",batFont));
				    vat.setHorizontalAlignment(Element.ALIGN_RIGHT);
				    vat.setBorder(Rectangle.NO_BORDER);
				    subChildTable.addCell(vat);
				   
				    
				    PdfPCell vatData=new PdfPCell(new Phrase(String.valueOf(precision.format(invoiceTax.getTaxableAmt())),batFont));
