...
public class TaxController {
	...
	@RequestMapping(value="/taxDetailsByRegion/{tax_region}", method = RequestMethod.GET, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<TaxInfo>>getTaxDetailsByRegion(@PathVariable("tax_region") String tax_region) {
	
		List<TaxInfo> taxList = service.findTaxByRegion(tax_region);
		System.out.println("getTax() called");
		if(taxList.isEmpty()){
			System.out.println("Tax List Empty");
			
			return new ResponseEntity<List<TaxInfo>>( HttpStatus.NO_CONTENT);
		}
