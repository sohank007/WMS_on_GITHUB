...
WMS.run(function($window){
	 $window.localStorage['orgId'] = "ORG-11";
});
/*WMS.run(function($http,$rootScope, $location,$window,auth) {
	  var ip="http://192.168.100.9:1337/users/";     // local pfactory
	...
	.error(function(data, status, headers, config) {
	  debugger;
	    if(status == 404)
	    {
	        $window.location.href = "http://192.168.100.9:1337/#/home";
	  ...
	  //$window.location.href = "http://localhost:1337/#/home";
	   ...
	   if($window.location.href.split("/")[3].length>0)
	   {
	    ...
	    $rootScope.$on('$routeChangeStart', function(event) {
	      if (!auth.isLoggedIn()) {
	        console.log($window.location.href.split("/")[3].length);
	        debugger;
